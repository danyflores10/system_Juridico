...
from apps.modificador.models import LeyOriginal, LeyResultado, ModificacionDocumento

from .extractor import contiene_abrogacion
from .guardado import guardar_resultado_automatico
...
def procesar_modificacion_db(mod: ModificacionDocumento) -> LeyResultado | None:
    ...
    # Guardado automático (Word + PDF con nomenclatura oficial). Si falla,
    # se registra como alerta en el pre-informe para revisión manual.
    guardado = guardar_resultado_automatico(lr)
    preinforme = lr.preinforme or {}
    preinforme['guardado_automatico'] = guardado
    if 'error' in guardado:
        preinforme.setdefault('errores_detectados', []).append(
            {
                'nivel': 'critico',
                'codigo': 'GUARDADO_FALLIDO',
                'mensaje': f"No se pudo guardar el archivo consolidado: {guardado['error']}",
                'icono': '🔴',
                'etiqueta': 'ERROR CRÍTICO',
                'articulo': '',
                'norma': mod.archivo_origen,
            }
        )
    lr.preinforme = preinforme
    lr.save(update_fields=['preinforme'])
    return lr


def procesar_todas_pendientes() -> dict:
    """Procesa modificatorios vinculados y no procesados. Retorna resumen."""
    ...
    for mod in mods:
        ...
        except Exception as e:  # noqa: BLE001 — resumen para el usuario
