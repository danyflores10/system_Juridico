"""Módulo 2 — Modificador jurídico.

Modelos del flujo: norma original → documento modificatorio → norma resultante.
"""
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='modificador_uniq_ley_activa_por_codigo',
            ),
        ]
...
class LeyResultado(models.Model):
    """Versión consolidada tras aplicar modificaciones (texto anterior tachado)."""
    ...
    es_version_final = models.BooleanField('Versión final aprobada', default=False)
    ...
    archivo_docx = models.CharField(
        'Ruta Word generado', max_length=500, blank=True
    )
    archivo_pdf = models.CharField(
        'Ruta PDF generado', max_length=500, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['ley_original', 'version'],
                name='modificador_uniq_resultado_version',
            ),
        ]
    ...
    def alertas_criticas(self) -> int:
        return len((self.preinforme or {}).get('errores_detectados', []))

    @property
    def advertencias_count(self) -> int:
        return len((self.preinforme or {}).get('advertencias', []))
