...
/**
 * Convierte "#rrggbb" en "r, g, b" para componer rgba() con alfa variable.
 * Si el color no es hexadecimal de 6 dígitos, cae en blanco (el valor que
 * usaba el componente antes de respetar lineColor).
 */
const hexARgb = (color: string): string => {
...
interface NeuralLinkProps {
  ...
  className?: string;
  /**
   * Fuerza la paleta clara u oscura en lugar de seguir el tema de la aplicación.
   * Necesario cuando el contenedor tiene su propio fondo fijo (por ejemplo el
   * panel azul marino del acceso, que es oscuro aunque la app esté en claro).
   */
  darkPalette?: boolean;
}
...
const NeuralLinkBackground: React.FC<NeuralLinkProps> = ({
  ...
  useEffect(() => {
    // Con la paleta forzada no hace falta observar el tema de la aplicación.
    if (darkPalette !== undefined) {
      isDarkModeRef.current = darkPalette;
      return;
    }
  ...
  useEffect(() => {
    ...
    const render = () => {
      // Fuera de vista se detiene el bucle en lugar de encadenar fotogramas
      // vacíos: el efecto vuelve a ejecutarse (isInView está en sus
      // dependencias) y lo reanuda en cuanto el fondo se vuelve visible.
      if (!isInView) return;
      ...
      // En paleta oscura se respeta lineColor (antes quedaba fijo en blanco y
      // la prop no tenía ningún efecto); en clara se mantiene el índigo.
      const finalLineStyle = isDark ? hexARgb(lineColor) : "79, 70, 229";
