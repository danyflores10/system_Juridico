...
import { AlertTriangle, ArrowRight, CheckCircle2, FileSearch, RefreshCw, Search, ShieldAlert } from "lucide-react";
...
const views: { value: VistaRevision; label: string; description: string; icon: typeof FileSearch }[] = [
  ...
  { value: "BAJA_CONFIANZA", label: "Datos por verificar", description: "Información que debe confirmarse", icon: ShieldAlert },
];
    ...
    <TableRow className="group cursor-pointer" onClick={onOpen}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              {item.numero_propuesto ? <span>Norma N.º {item.numero_propuesto}</span> : <span>Número no detectado</span>}
            </div>
          </div>
        </div>
      </TableCell>
      <TableCell>
        {item.alertas_activas ? (
          <Badge variant="outline" className={cn("gap-1", item.alertas_graves_activas ? "border-red-300 bg-red-50 text-red-700 dark:bg-red-950/30" : "border-amber-300 bg-amber-50 text-amber-700 dark:bg-amber-950/30")}>
            <AlertTriangle className="size-3.5" /> {item.alertas_activas} observaciones
          </Badge>
        ) : item.campos_baja_confianza ? <span className="text-sm text-amber-700">Confirmar datos encontrados</span> : <span className="text-sm text-emerald-700">Verificar y aprobar ficha</span>}
      </TableCell>
      <TableCell><DocumentoStatusBadge estado={item.estado} /></TableCell>
      <TableCell>
      ...
      <TableCell className="text-right">
        <Button variant="ghost" size="sm" className="group-hover:bg-foreground group-hover:text-background" onClick={(event) => { event.stopPropagation(); onOpen(); }}>
    ...
    <div className="flex flex-col gap-5">
      ...
      <Card className="gap-0 py-0">
        <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-md"><Search className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} className="pl-9" placeholder="Buscar por código, título o número…" /></div>
          <Button variant="outline" size="sm" onClick={() => void queue.refetch()} disabled={queue.isFetching}><RefreshCw className={cn("size-4", queue.isFetching && "animate-spin")} /> Actualizar</Button>
        </div>
        {queue.isError ? <div className="m-4 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-destructive">{getApiErrorMessage(queue.error)}</div> : null}
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow><TableHead className="pl-4">Documento</TableHead><TableHead>Qué necesita</TableHead><TableHead>Estado</TableHead><TableHead>Recepción</TableHead><TableHead className="pr-4 text-right">Acción</TableHead></TableRow></TableHeader>
            <TableBody>
              {queue.isLoading ? ["a", "b", "c", "d", "e"].map((key) => <TableRow key={key}><TableCell colSpan={5} className="p-4"><Skeleton className="h-11 w-full" /></TableCell></TableRow>) : null}
              {!queue.isLoading && !queue.data?.results.length ? <TableRow><TableCell colSpan={5} className="h-56 text-center"><div className="mx-auto grid size-12 place-items-center rounded-full bg-muted"><CheckCircle2 className="size-5 text-muted-foreground" /></div><div className="mt-3 font-medium">No hay documentos en esta vista</div><div className="mt-1 text-sm text-muted-foreground">Cambie el filtro o actualice la bandeja.</div></TableCell></TableRow> : null}
              {queue.data?.results.map((item) => <ReviewRow key={item.uuid} item={item} onOpen={() => router.push(`/dashboard/revision-juridica/${item.uuid}`)} />)}
            </TableBody>
          </Table>
        </CardContent>
        <div className="flex items-center justify-between border-t px-4 py-3 text-sm text-muted-foreground">
          <span>{queue.data?.count ?? 0} documentos · Página {page} de {pageCount}</span>
          <div className="flex gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Anterior</Button><Button variant="outline" size="sm" disabled={page >= pageCount} onClick={() => setPage((value) => value + 1)}>Siguiente</Button></div>
        </div>
      </Card>
    </div>
  );
