...
from .html_formatter import asegurar_html, estructurar_texto_legal
...
def extraer_texto_archivo(archivo) -> tuple[str, str]:
    """
    Extrae contenido como HTML estructurado.
    Retorna (html_contenido, extension).
    """
    ...
    if ext in (".html", ".htm"):
        texto = _leer_texto(raw)
        return asegurar_html(texto), ext
...
def _extraer_docx_html(raw: bytes) -> str:
    ...
    from .html_formatter import envolver_documento
